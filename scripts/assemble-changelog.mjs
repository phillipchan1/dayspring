...
const REPO = process.env.RELEASES_REPO || 'phillipchan1/dayspring-releases'
...
const tagToVersion = (t) => (t || '').replace(/^app-v/, '')
...
async function fetchReleases() {
  const headers = { accept: 'application/vnd.github+json', 'user-agent': 'dayspring-changelog' }
...
// API's release order isn't reliable, but our versions are monotonic 0.1.<run> —
// sort by that run number descending so it's always strictly newest-first.
