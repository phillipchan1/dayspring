...
//
// ios-release.yml derives its build number from the commit count on the built
// ref (`git rev-list --count HEAD`) — strictly increasing per commit, and
// independent of any workflow's own run counter. That's unique per commit but
// not per *upload attempt*: a manual re-run of the workflow on a commit that
// already uploaded would reuse the same CFBundleVersion, and App Store Connect
// rejects an upload whose build number isn't strictly greater than the last one
// for that marketing version (CFBundleShortVersionString).
//
...
// max(proposed, existingMax + 1) to stdout. The common case (first upload of a
// commit) has no prior build, so it just echoes the proposed number back. Only
// a genuine retry-on-the-same-commit diverges, and only by the minimum needed
// to satisfy Apple.
//
// BEST-EFFORT: any API/auth problem falls back to the proposed number (with a
// stderr warning) rather than failing the build — a transient App Store Connect
// hiccup shouldn't block a release the common case doesn't actually need it for.
//
// Usage:
//   node scripts/appstore-next-build-number.mjs \
//     --bundle-id com.phillipchan.dayspring --version 1.0.488 --proposed 488 \
...
function fallback(reason) {
  process.stderr.write(
    `[appstore-next-build-number] falling back to proposed (${proposed}): ${reason}\n`,
  )
...
function makeJwt() {
  ...
  const payload = base64url(
    JSON.stringify({ iss: issuerId, iat: now, exp: now + 1140, aud: 'appstoreconnect-v1' }),
  )
...
try {
  ...
  const apps = await get(
    `${base}/apps?filter[bundleId]=${encodeURIComponent(bundleId)}&limit=1`,
    token,
  )
  const appId = apps.data?.[0]?.id
  if (!appId) throw new Error(`no app found for bundle id ${bundleId}`)

  // The app-nested relationship listing (/v1/apps/{id}/preReleaseVersions) only
  // accepts a narrow filter set and rejects filter[version] outright ("A given
  // parameter is not allowed for this request"). The top-level collection
  // endpoint supports filtering by both app and version together.
