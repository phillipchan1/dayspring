...
const spiritualBlockTheme = EditorView.theme({
  ...
  /*
   * The rule, the indent and the air are custom properties because each of the
   * six voices tunes them — Vellum wants a heavier rule, Sabbath wants none at
   * all and the space instead, Plainsong wants it dashed. Those overrides live
   * in themes.css beside every other per-voice decision (and have to: they key
   * off `[data-theme]`, which is on <html>, not inside the editor).
   *
   * They used to be `border-left` overrides on the block widget. A border was
   * never right here — it runs the full height of every line, so two quotations
   * on consecutive lines drew one unbroken bar — and a background image can be
   * sized short of the box, which is what gives a run ends. The trade is that
   * "dashed" is no longer a keyword, so a voice overrides the whole image.
   */
  '.cm-scripture-line': {
    ...
    cursor: 'text',
    paddingLeft: 'var(--scripture-indent, 1rem)',
    paddingRight: '1.6rem',
    backgroundImage: 'var(--scripture-rule-image, none)',
    backgroundSize: 'var(--scripture-rule-width, 1px) 100%',
  ...
  '.cm-scripture-line--first': {
    position: 'relative',
    paddingTop: 'var(--scripture-air, 0.3em)',
    backgroundPosition: '0 var(--scripture-air, 0.3em)',
    backgroundSize: 'var(--scripture-rule-width, 1px) calc(100% - var(--scripture-air, 0.3em))',
  ...
  '.cm-scripture-line--first.cm-scripture-line--last': {
    backgroundSize:
      'var(--scripture-rule-width, 1px) calc(100% - var(--scripture-air, 0.3em))',
  },
  ...
  '.cm-scripture-cite': {
    paddingLeft: 'var(--scripture-indent, 1rem)',
    paddingBottom: 'var(--scripture-air, 0.3em)',
    backgroundImage: 'var(--scripture-rule-image, none)',
    backgroundSize:
      'var(--scripture-rule-width, 1px) calc(100% - var(--scripture-air, 0.3em))',
    backgroundPosition: '0 0',
...
function blockAtEvent(
  view: EditorView,
  // Coordinates only — served both a `TapContext` from `editorTap` and a plain
  // MouseEvent from the verse's click handler.
...
/**
 * Tap a rendered marking to edit it, or a quotation to read around it.
 *
 * Resolves the block fresh from the live document (positions stay correct even
 * after edits above it) and hands the caller its range + contents so it can
 * reopen the matching popover. Uses press-coordinate → document position
 * lookup so that two blocks sharing the same UUID (e.g. copy-pasted) are each
 * resolved to their own range rather than always resolving to the first.
 *
 ...
 *
 * ## The verse is the one thing here that is not claimed
 *
 * `claims` returning true calls `preventDefault()` on the press, and that is
 * precisely what stops a drag becoming a selection and a long-press raising
 * the selection callout. Those two gestures are how a phrase inside a verse
 * gets marked, so the verse cannot be claimed without deleting the feature.
 *
 * Its tap is served by the plain `click` handler below instead, which runs
 * after the browser has decided what the gesture was: a selection means a mark
 * is being made and the chapter stays shut; no selection means it was a tap,
 * and the chapter opens exactly as D-024 decided.
 *
 * **The cost, stated rather than hidden:** on iOS a tap on the verse is served
 * by the compatibility `click`, so it keeps the ~300ms lag `editorTap` exists
 * to remove. It buys selectable scripture, it is confined to "open the
 * chapter", and nothing on the writing path pays it. The citation — which is
 * the advertised door, and is claimed — stays instant.
 */
function blockTapHandler(
  ...
  const parts = (target: EventTarget | null) => {
    const el = target as HTMLElement | null
    const menuEl = (el?.closest('.cm-scripture-menu') ?? null) as HTMLElement | null
    const citeEl = (el?.closest('.cm-scripture-cite') ?? null) as HTMLElement | null
    const verseEl = (el?.closest('.cm-scripture-line') ?? null) as HTMLElement | null
    const markLineEl =
      menuEl || citeEl || verseEl ? null : ((el?.closest('.cm-mark-line') ?? null) as HTMLElement | null)
    return { el, menuEl, citeEl, verseEl, markLineEl }
  }
  ...
  const open = (
    ...
    if (wantEdit || !onOpenChapter) onEdit(target, anchor)
  ...
  return [
    editorTap({
      claims: (ctx) => {
        const { menuEl, citeEl, verseEl, markLineEl } = parts(ctx.target)
        if (menuEl || citeEl) return true
        // See the note above: claiming the verse would preventDefault the press
        // and there would be nothing left to select.
        if (verseEl) return false
        return Boolean(markLineEl)
      },
      onTap: (ctx, view) => {
        const { el, menuEl } = parts(ctx.target)
        const block = blockAtEvent(view, ctx, el)
        if (!block) return false
        return open(view, block, block.type !== 'scripture' || Boolean(menuEl))
      },
    }),

    EditorView.domEventHandlers({
      click(event, view) {
        if (!onOpenChapter) return false
        const { el, verseEl, menuEl } = parts(event.target)
        if (!verseEl || menuEl) return false
        // A selection means the reader is marking, not leaving.
        if (!view.state.selection.main.empty) return false
        const block = blockAtEvent(view, event, el)
        if (!block || block.type !== 'scripture') return false
        return open(view, block, false)
      },
    }),
  ]
}
 ...
 *
 * That break is reachable from the line above without the writer aiming at it:
 * Backspace from the start of the fence line means "go up a line", and the line
 * up is exactly this newline. Delete from the end of the line above, or a
 * selection that runs down onto the fence, swallow it the same way. (It used to
 * be reachable from an empty-looking stub row the block widget left above
 * itself; no kind renders as a replace widget any more, so that row is gone and
 * the caret simply starts on the fence line instead. The protection is the
 * same either way — a fence is only a fence at a line start.)
 *
