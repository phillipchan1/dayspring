...
import { coverPhotos } from './Shelf'
import { spanName, type Volume } from './volumes'
import './Volumes.css'
...
export function ClosingMoment({
  ...
  return createPortal(
    <div className="vol-closing" role="dialog" aria-modal="true" aria-label={`${spanName(volume.from, volume.to)} is full`}>
      ...
      <div className="vol-closing__card">
        ...
        <h2>This one is full.</h2>
        <p className="vol-closing__sub">
          {spanName(volume.from, volume.to)}. You wrote its last page on{' '}
        ...
        {carried.length > 0 ? (
          <div className="vol-closing__carry">
            <span className="vol-eyebrow">Still going as it closes — carried into the next</span>
            <p>{carried.join(' · ')}</p>
          </div>
        ) : null}
        ...
        <p className="vol-closing__fine">Nothing changes where you write. Your next page begins the next one.</p>
      </div>
    </div>,
    document.body,
  )
