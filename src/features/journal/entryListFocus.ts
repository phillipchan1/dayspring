/**
 * One entry row, in px — the list's own geometry.
 *
 * It used to be the default inside `useVirtualRange`. It moved here when the
 * Pages wall became a second consumer with a completely different row height:
 * a shared default is how the windowing math and the CSS drift apart, so each
 * caller now owns its number and passes it explicitly.
 */
