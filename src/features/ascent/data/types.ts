...
// ── Summit stones (REAL) ───────────────────────────────────────────────────────
// The yearly rollup has always produced these — an earlier ask paired with a
// later moment, both copied verbatim and both validated against real entries —
// and nothing has ever rendered them. They are the year's only true
// accumulation: a stone appears when an ask meets an answer, which is an event
// in a life, never a reward for writing.

/** One end of a stone: a verbatim line, with the entry it came from. */
export interface StoneMark {
  entryId: string
  /** YYYY-MM-DD — places the stone along the trail. */
  date: string
  dateLabel: string
  text: string
}
...
export interface SummitStone {
  ...
  ask: StoneMark
  later: StoneMark
  /** 0–1 along the year, taken from the LATER date: the stone is set when the
   *  answer arrives, not when the asking began. */
  position: number
}

/** The long look — who you were against who you are now, and the year's threads
 *  in prose. Generated by the yearly pass, and folded away until it is asked
 *  for: at the Summit the app goes quiet, which governs what it says unbidden,
 *  not what it keeps within reach. */
export interface LongLook {
  throughline: string[]
  themes: string[]
}
...
/** The Summit adds the stones on the trail, the folded long look, and how far
 *  into the year the climb has actually come. */
export interface SummitView extends AltitudeData {
  year: number
  stones: SummitStone[]
  longLook: LongLook | null
  /** 0–1 of the calendar year elapsed. Tracks the CALENDAR, never the writer:
   *  it reaches the peak on Dec 31 whether the year held four hundred entries or
   *  four. The moment it moves faster because someone wrote more, it is a streak
   *  wearing a metaphor. */
  progress: number
}
