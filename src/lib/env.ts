...
export const env = {
  ...
  /**
   * PostHog project API key. Unset → the usage transport never loads and
   * events stay a dev-console no-op (see lib/analytics/posthog.ts).
   *
   * There is deliberately no host var to go with it. The SDK talks to our own
   * `/cairn` path, which vercel.json proxies — so the destination is a
   * deploy-time concern in one file, not a build-time one baked into every
   * desktop binary we have ever shipped.
   */
