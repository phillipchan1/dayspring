...
describe('parseReferences — a reference is written on one line', () => {
  // Found by writing a LONGER demo entry, which is the only kind that tests
  // this: a short body has no line for a book name to sit at the end of.
  it('does not marry a name at a line end to the next line’s list index', () => {
    // Was: Dan.4 at confidence 1.0 — a to-do list, and the most ordinary shape
    // a journal has.
    expect(
      osis('2. call the roofer back\n3. actually reply to Dan\n4. Frontier budget'),
    ).toEqual([])
  })

  it('does not reach across blank lines to an imported footer', () => {
    expect(osis('I want to do better than that by Esther\n\n\n57.2°F ☁️')).toEqual([])
    expect(osis('loving reading Jeremiah\n\n\n\n\n48.1°F ☁️')).toEqual([])
  })

  it('still reads a reference spaced across one line', () => {
    expect(osis('sat with John  3 : 16  for a while')).toEqual(['John.3.16'])
    expect(osis('Romans chapter 8 this morning')).toEqual(['Rom.8'])
  })
})
