...
//
// The whitespace between the name and its chapter is HORIZONTAL ONLY. A
// reference is written on one line; `\s*` let a book name at the end of a
// paragraph reach across blank lines and marry whatever number came next, which
// is how an imported weather footer ("…and Esther\n\n\n57.2°F") became Esth.10.2
// and how an ordinary to-do list —
//
//     3. actually reply to Dan
//     4. Frontier budget
//
// — became Dan.4 at confidence 1.0, indistinguishable downstream from a verse
// the writer actually read. Measured over the corpus, the restriction costs no
// recall at all (0.835 either way) and takes explicit precision to 1.000.
const REF_RE = new RegExp(
  String.raw`\b(${NAME_ALT})\.?[^\S\n]*(ch(?:apter|apters|aps?)?\.?[^\S\n]*)?` +
    String.raw`(\d{1,3})` +
