...
const DECLARED_KINDS: MarkingKind[] = ['scripture', 'sense', 'prayer', 'desire', 'learned', 'story']
...
/*
 * `giftBefore` / `consolations` lived here until 2026-08-26.
 *
 * They paired a declared Absence with the last declared Gift before it —
 * Ignatius' instruction to remember, from inside a dry season, that the
 * consolation was real. RECALL calls it the strongest of the six contemplative
 * mechanisms, because it removes manual labour from a practice that already
 * exists and invents nothing: she marked both ends, and the app writes nothing
 * between them.
 *
 * Both kinds were cut from the palette on 2026-08-26 — a writer read the
 * labels and did not know what they meant — and the arrangement cannot be
 * rebuilt without them. Recorded here rather than silently deleted, because
 * the idea is sound and it was the VOCABULARY that failed. If Absence ever
 * comes back under a word people recognise, this is the thing to rebuild.
 */
