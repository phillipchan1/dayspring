// ============================================================
// /maker — "A note from the maker". A personal founder note out-
// converts a corporate team grid for this audience. First-person,
// contemplative, to the First Light guardrails.
//
// ⚠️ DRAFT FOR PHIL TO MAKE HIS OWN. Grounded only in what's true
// from Phil (journaling since about 2001 — 25 years; built it for himself
// first; the year-in-review he could never get elsewhere). Personal
// specifics are intentionally light — add your real ones, and confirm
// the signature.
// ============================================================

import { downloads } from "./site";

export const maker = {
  eyebrow: "A note from the maker",
  title: "I built this for myself first.",
  paragraphs: [
    "I didn't set out to build a product. I set out to solve a problem I'd had for twenty-five years.",
    "I've kept a journal for twenty-five years — thousands of entries, across more apps than I'd care to admit, through every kind of season: the slow, unremarkable stretches and the ones that knocked the wind out of me. And in all of that writing, I could never actually <em>see</em> anything. Each entry sat alone in its own day. I'd reread an old one and barely recognize the person who wrote it — but I could never trace the line between him and me.",
    "That's the strange thing about the inner life: the changes that matter most are the ones you can't feel happening. Patience doesn't arrive on a Tuesday. Prayer doesn't deepen on a schedule. You only notice in hindsight — if you notice at all.",
    "I wanted a journal that remembered for me. One that could read back a year of my own words and tell me, honestly, what had shifted — not a mood chart, not a streak, but something closer to a letter from a friend who'd been paying attention the whole time. Measured against who I actually was, not the person I keep thinking I should already be.",
    "So I built it — first just for me. The page I'd always wanted to write on, and the year-in-review I'd never managed to get any other way. It turned out the people I most wanted to hand it to were the ones who, like me, are quietly trying to pay attention to their formation before God. Without a brochure. Without being shouted at.",
    "Dayspring is that journal. I hope it holds your words as carefully as I've tried to make it hold mine.",
  ],
  signature: "— Phil Chan",
  /** Under the signature. The credential, said once. */
  byline: {
    before: "Author of ",
    book: "Rhythms of Resilience: Faith Practices to Thrive in Chaos",
    href: "https://www.amazon.com/dp/1957321105",
  },
  cta: { label: "Download for macOS", href: downloads.macos.href },
};
